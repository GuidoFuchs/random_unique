...
impl RandomSequenceOfUniqueU32 {
    ...
    fn permutate(&self, x: u32) -> u32 {
        ...
        if x <= PRIME_HALF {remainder} else {PRIME - remainder}
    }
    ...
    pub fn new(seed_base: u32, seed_offset: u32) -> Self {
        ...
        s.re_seed(seed_base, seed_offset);
        s
    }

    pub fn re_seed(&mut self, seed_base: u32, seed_offset: u32) {
        // actual numbers don't matter to much
        const SHIFT_INDEX: u32 = 0x682f0161;
        const SHIFT_OFFSET: u32 = 0x46790905;

        self.index =  self.permutate(self.permutate(seed_base).wrapping_add(SHIFT_INDEX));
        self.offset = self.permutate(self.permutate(seed_offset).wrapping_add(SHIFT_OFFSET));
    }
    ...
    pub fn current(&self) -> u32 {
        ...
        self.permutate((self.permutate(self.index).wrapping_add(self.offset)) ^ XOR_CONST)
    }

    pub fn next_nr(&mut self) -> u32 {
        // Linearly counting up through all the random numbers
        self.index = self.index.wrapping_add(1);
        self.current()
    }

    pub fn scramble_number(&mut self, n: u32) -> u32 {
        self.index = n;
        self.current()
    }
...
mod tests {
    ...
    fn test_for_no_repeat(){
        ...
        let mut a =  Box::<BitArray>::default();
    ...
    // Below is only implementation of test code 
    // Namely of the BitArray that is used to test the random number generation
    // It is for testing, that no number is repeating for 0xFFFFFFFF cycles
    ...
    fn test_set_bit_if_not_set() {
        let mut a =  Box::<BitArray>::default();
        assert!(a.set_bit_if_not_already_set(3));
        assert!(!a.set_bit_if_not_already_set(3));
